...
void buildCodeMap(int idx, const std::vector<Node>& nodes, const std::string& prefix, std::unordered_map<unsigned char,std::string>& codeMap);
...
void encodeData(std::istream& in, class BitWriter& writer, const std::unordered_map<unsigned char,std::string>& codeMap);

void decodeData(class BitReader& reader, std::ostream& out, int rootIdx, const std::vector<Node>& nodes);
